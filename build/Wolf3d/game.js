...
Wolf.Game = (function () {
  ...
  function updateStat(name, value) {
    const numdivs = $('#game .hud .' + name + ' .number');
  ...
  function updateScreen(game) {
    ...
    const res = Wolf.Raycaster.traceRays(viewport, level);
  ...
  function startLevel(game, episodeNum, levelNum) {
    ...
    const episode = Wolf.Episodes[game.episodeNum];

    Wolf.Level.load(episode.levels[game.levelNum].file, (error, level) => {
      if (error) {
        throw error;
      }
  ...
  function startIntermission(game, delay) {
    let episode = Wolf.Episodes[game.episodeNum],
