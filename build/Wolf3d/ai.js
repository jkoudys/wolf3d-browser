...
Wolf.AI = (function () {
  ...
  function path(self, game) {
    ...
    if (level.tileMap[self.x >> Wolf.TILESHIFT][self.y >> Wolf.TILESHIFT] & Wolf.WAYPOINT_TILE) {
      const tileinfo = level.tileMap[self.x >> Wolf.TILESHIFT][self.y >> Wolf.TILESHIFT];
  ...
  function T_Shoot(self, game, tics) {
    ...
    const trans = Wolf.Math.transformPoint(self.x, self.y, player.position.x, player.position.y);
  ...
  function T_BJRun(self, game, tics) {
    const move = Wolf.BJRUNSPEED * tics;
