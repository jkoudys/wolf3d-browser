...
Wolf.Menu = (function () {
  ...
  function setupEvents() {
    $(document).on('keydown', (e) => {
      ...
      if (oldActive != activeIndex) {
        const items = $('#menu div.menu.active li:not(.hidden)');
        if (activeIndex < 0) {
    ...
    $('#menu li').on('click', function (e) {
      ...
      if ($this.hasClass('mouseenabled')) {
        const mouseOn = Wolf.Game.isMouseEnabled();
    ...
    $('#menu div.menu.episodes li').on('click', function (e) {
      ...
      if (Wolf.Game.isPlaying()) {
        showMessage('confirm-newgame', true, (result) => {
          if (result) {
    ...
    $('#menu div.menu.main li.resumegame').on('click', (e) => {
      if (!menuInputActive) {
        return;
      }
      if (Wolf.Game.isPlaying()) {
    ...
    $('#menu div.menu.main li.readthis').on('click', (e) => {
      if (!menuInputActive) {
        return;
      }
      menuInputActive = false;
  ...
  function customizeKeys($this) {
    ...
    function selectKey(index) {
      if (index < 0) index += 4;
      index = index % 4;
      const currentSprite = $('span.active', $this);
      if (currentSprite[0]) {
        setCustomizeKey(
      ...
      const sprite = $('span.k' + (index + 1), $this);
      setCustomizeKey(
                sprite.data('action'),
    ...
    function activateKey(index) {
      ...
      let sprite = $('span.k' + (index + 1), $this),
        blink = false;
  ...
  function setCustomizeKey(action, keyIndex, active) {
    let menu = $('#menu div.menu.customize'),
      x = (active ? -256 : 0),
  ...
  function initCustomizeMenu() {
    let controls = Wolf.Game.getControls(),
