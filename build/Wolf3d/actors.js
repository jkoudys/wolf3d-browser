...
Wolf.Actors = (function () {
  ...
  function spawnDeadGuard(level, skill, which, x, y) {
    const self = spawn(level, skill, which, x, y, Wolf.Math.dir4_nodir);
    if (!self) {
      return;
    }
  ...
  function spawnPatrol(level, skill, which, x, y, dir) {
    const self = spawn(level, skill, which, x, y, dir);
    if (!self) {
      return;
    }
  ...
  function spawnBJVictory(player, level, skill) {
    let x = Wolf.POS2TILE(player.position.x),
